...
Ok, new day
Mostly still working except for some reason, autosnails only runs the 2nd time you run the game
Not really sure why

Talked to Rachel, it was because the clock function was inside the autoSnails function, so it wasn't going on the server until the 2nd time I ran it
Oopsie
...
~buyAutoSnails = {
	if (counter > factoryCost, {
		...
		factoryCost = factoryCost + (factoryCost/2);
		d.string = "Buy Factory for " + factoryCost + "🐌: (" + factories + ")";
		if (factories == 1, {~autoSnails.value; "called AutoSnails".postln; counter = counter + 1;});
	}, {e.string = "Not enough 🐌"; ~errorMessage.value;});
};
...
Next thing I want to do is simplify the if statements for the achievements
I want to try having the if statement be a variable that then runs a function that'll update the variable so we only run each achievement 1x
And we can keep adding layers without having to have 50 if statements and bool flags and stuff


trying the following bits of code

```
var curAchievement = "if (counter > 5, {~nextAchievement;});";

//counter update
curAchievement.interpret;
...
```
this is throwing an error that the counter variable doesn't exist
so that's cool

also tried both of the following
```
/*nextAchievement = curAchievement.asCompileString;
	if (nextAchievement == true, {"nextAchievementTrue".postln; ~nextAchievement.value;});*/
	//if (curAchievement == true, {~nextAchievement.value;});
```

and none of this is working tragically

basically I don't want the if statements to run again once they've been satisfied
Some options:
- having a "current" if statement that is changed to be the next if statement whenever the if function runs
	- what i'm trying now
	- is not working
- somehow deleting or ignoring if statements that have already run
	- can't think of how to do this

If this were unity I'd try having an array of if statements and the array number just ticks up each time 
but idk

Maybe we could do an array of strings
Compare the current string to the array
And if it matches any of them, and that thing is true, then run audio layers and tick up the array

so like:

string array myStringArray ["string 1", "string 2"]
curAchievement = 0;
switch (myStringArray[curAchievement],
myStringArray[0], {if (counter > 5, {~audioLayers.value; curAchievement = curAchievement + 1;})};
)

I guess something like that?
https://doc.sccode.org/Classes/String.html
...
```
I gave up on strings and I'm just doing an integer alas
Just added an ~audioLayers.value to when you buy your first factory

ok so now I want to make a new function
Maybe we can buy a snail manager that will auto buy stuff when you can afford it
Also I want to make the snail text bigger
https://docs.supercollider.online/Classes/StaticText.html
